...
tfrecords_filename ='/media/hdd/csie/features/tmp_feat/train.segtree.tfrecords'
...
def _parse_function(example_proto):
    ...
    sequence_features = {
             ...
             'bnds': tf.FixedLenSequenceFeature([], dtype=tf.int64)
         }
    ...
    bnds_parsed = sequence_parsed['bnds']
    #sequence_parsed.set_shape((context_parsed['length'], context_parsed['feat_dim']))
    return feats_parsed, bnds_parsed, tf.expand_dims(seq_len, axis=0)
...
def inputs(batch_size, tfrecords_filename):
    ...
    dataset = dataset.padded_batch(batch_size, padded_shapes=([None, None], [None], [None]))
