...
def gen_bnd_seq_with_ctm(uttid, bnd_sequence, ctm_fp=None):
    ...
    while True:
        ...
        if line.rstrip().split()[0] != uttid:
            new_bnd_sequence = [int(line.rstrip().split()[1])]
            new_uttid = line.rstrip().split()[0]
            return bnd_sequence, new_uttid, new_bnd_sequence

        else:
            bnd_sequence.append(int(line.rstrip().split()[1]))
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 4:
        print('Usage: ./gen_tfrecords.py <scp_file> <output_tfrecords_filename> <ali_file>')
