...
def batch_pair_data(feats, spk2feat, feat2label, feat_indices, spk_list):
    ...
    try:
        arr1 = np.array(batch_data, dtype=np.float32)
        arr2 = np.array(batch_data_pos, dtype=np.float32)
        arr3 = np.array(batch_data_neg, dtype=np.float32)
    except:
        pdb.set_trace()
