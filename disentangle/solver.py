...
class Solver(object):
    ...
    def generate_opt(self, loss, learning_rate, momentum, var_list):
        ...
        capped_gvs = [(grad if grad is None else tf.clip_by_value(grad, -1., 1.), var) for grad, var in gvs]
    ...
    def save_batch_BN(self, word_word_dir, word_spk_dir, spk_word_dir, spk_spk_dir, phonetic_file,
        ...
        for i in range(len(feat_indices)):
            utter = self.feat2label_test[feat_indices[i]][0]
    ...
    def compute_loss(self, mode, sess, summary_writer, summary_op, epoch, reconstruction_loss, generation_loss,
        ...
        summary = None
        if mode == 'train':
            modified_n_batches = n_batches
        else: modified_n_batches = n_batches + 1

        for step in tqdm(range(modified_n_batches)):
            ...
            # Pad the three elements
            # self.batch_size is the set batch_size
            if self.batch_size > batch_size:
                paddings = np.ones((self.batch_size - batch_size, self.seq_len, self.feat_dim))
                batch_examples = np.concatenate([batch_examples, paddings], axis=0)
                batch_examples_pos = np.concatenate([batch_examples_pos, paddings], axis=0)
                batch_examples_neg = np.concatenate([batch_examples_neg, paddings], axis=0)
            ...
            if mode == 'train':
                ...
                # Remove paddings
                if self.batch_size > batch_size:
                    p_memories = p_memories[:batch_size]
                    s_memories = s_memories[:batch_size]
    ...
    def train(self):
        ...
        ### Load data  ###
        feats_dir = os.path.join(self.feat_dir, str(self.seq_len))
