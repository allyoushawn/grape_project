    ...
    with open(filename,'r') as f:
        for line in f:
            ...
            if ID not in classify_dic:
                classify_dic[ID] = []
            classify_dic[ID].append((start_frame,cont_frame))
...
def read_and_save_feat(prons, filename, classify_dic, frame_num_list, path, feat_dim):
    import csv 
    import numpy as np
    counter_dic = {}
    for i in frame_num_list:
        ...
        for line in f:
            if '[' in line:
                ...
                for start, cont in classify_dic[ID]:
                    ...
                    end_idx = int(start) + int(cont)

                    new_frames = [ temp_list[i] for i in \
                        range(int(start),end_idx)]
                    ### padding zero ###
                    #print (cont, cls)
...
def main():
    ...
    read_and_save_feat(FLAG.prons, FLAG.feat_ark, classify_dic, classify_list, path, FLAG.feat_dim)
    return 
